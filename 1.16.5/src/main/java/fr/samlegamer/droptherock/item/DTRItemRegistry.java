...
public class DTRItemRegistry {
    ...
    public static final MaterialTiers COPPER_TOOL_MATERIAL = new MaterialTiers(2, 300, 0.7F, 0.0F, 10, new LazyValue<>(() ->
    {return Ingredient.of(ForgeRegistries.ITEMS.getValue(new ResourceLocation("iceandfire", "copper_ingot")));}));

    public static final MaterialTiers DRAGONBONE_TOOL_MATERIAL = new MaterialTiers(4, 1660, 10.0F, 4.0F, 22, new LazyValue<>(() ->
    ...
    public static void registerCustom()
    {
        for(File file : Objects.requireNonNull(FMLPaths.CONFIGDIR.get().resolve("dropthemat"+File.separator).toFile().listFiles()))
        {
            ...
            try(JsonReader reader = new JsonReader(new FileReader(file)))
            {
                ...
                mat = new MaterialTiers(level, durability, efficiency, attack, enchant, new LazyValue<>(() ->
                {return Ingredient.of(ForgeRegistries.ITEMS.getValue(new ResourceLocation(finalRepair)));}));

                String name = file.getName().replace(".json", "");

                ITEM.register(name+"_knife", () -> new KnifeItem(mat, 1, -2.2F, new Item.Properties().tab(ModItemGroup.ITEMS)));
                ITEM.register(name+"_saw", () -> new AxeItem(mat, 2.0F, -3.2F, new Item.Properties().tab(ModItemGroup.ITEMS)));
                ITEM.register(name+"_mattock", () -> new MattockItem(mat, 0.5F, -3.0F, new Item.Properties().tab(ModItemGroup.ITEMS)));
            }
            catch (Exception e)
            {
    ...
    public static Item.Properties getPropertiesIsModLoaded(String modid) {
        ...
        if (ModList.get().isLoaded(modid)) {
            properties.tab(ModItemGroup.ITEMS);
        }
        return properties;
    }
}