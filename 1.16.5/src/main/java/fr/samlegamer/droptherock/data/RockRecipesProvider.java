...
public class RockRecipesProvider extends RecipeProvider
{
    ...
    protected void buildShapelessRecipes(Consumer<IFinishedRecipe> consumer)
    {
        ...
        for (Rock rock : rocks) {
            ...
            String[] partsCobble = rock.getCobblestone().split(":");
            String modidCobble = partsCobble[0];
            String nameCobble  = partsCobble[1];
            String[] partsRock = rock.getRock().split(":");
            String modidRock = partsRock[0];
            String nameRock  = partsRock[1];

            //DropTheRock.LOGGER.info("rocks {} {} {}", nameLoose, modidCobble, nameCobble);

            Block blockRock = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(modidRock, nameRock));
            Block blockLoose = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(DropTheRock.MODID, nameLoose));
            Block blockCobble = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(rock.getCobblestone()));
            Block blockCobbleSlab = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(rock.getCobblestone()+"_slab"));
            Block blockCobbleStairs = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(rock.getCobblestone()+"_stairs"));
            Block blockCobbleWall = ForgeRegistries.BLOCKS.getValue(new ResourceLocation(rock.getCobblestone()+"_wall"));
            ...
            try {
                if(blockLoose != null && blockCobble != null && !blockCobble.toString().contains(DropTheRock.MODID))
                {
                    ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modidRock))
                            .addRecipe(ShapedRecipeBuilder.shaped(blockCobble).group("rock")
                                    .unlockedBy("has_loose_rock", has(blockLoose))
                                    .define('#', blockLoose).pattern("##").pattern("##")::save).generateAdvancement()
                            .build(consumer, new ResourceLocation(DropTheRock.MODID, modidRock+"_"+nameCobble));
                }
                ...
                blockCobbleStairs != null && blockCobbleWall != null && blockCobble.toString().contains(DropTheRock.MODID))
                {
                    ...
                    ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modidRock))
                            .addRecipe(ShapedRecipeBuilder.shaped(blockCobble).group("rock")
                                    .unlockedBy("has_loose_rock", has(blockLoose))
                                    .define('#', blockLoose).pattern("##").pattern("##")::save).generateAdvancement()
                            .build(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble));
...
//                            .build(consumer, new ResourceLocation(DropTheRock.MODID, modidRock+"_"+nameRock + "_furnace"));
                    ...
                    //Slab Recipe
                    ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modidRock))
                            .addRecipe(ShapedRecipeBuilder.shaped(blockCobbleSlab, 6).group("slab")
                                    .unlockedBy("has_cobblestone", has(blockCobble))
                                    .define('#', blockCobble).pattern("###")::save).generateAdvancement()
                            .build(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_slab"));

                    //Stairs Recipe
                    ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modidRock))
                            .addRecipe(ShapedRecipeBuilder.shaped(blockCobbleStairs, 4).group("stair")
                                    .unlockedBy("has_cobblestone", has(blockCobble))
                                    .define('#', blockCobble)
                                    .pattern("#  ").pattern("## ").pattern("###")
                                    ::save).generateAdvancement()
                            .build(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_stairs"));

                    //Wall Recipe
                    ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modidRock))
                            .addRecipe(ShapedRecipeBuilder.shaped(blockCobbleWall, 6).group("wall")
                                    .unlockedBy("has_cobblestone", has(blockCobble))
                                    .define('#', blockCobble)
                                    .pattern("###").pattern("###")::save).generateAdvancement()
                            .build(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_wall"));

                    //Stone cutter recipes
                    SingleItemRecipeBuilder.stonecutting(Ingredient.of(blockCobble), blockCobbleSlab, 2)
                            .unlocks("has_stone", has(blockCobble))
                            .save(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_slab_stonecutting"));


                    SingleItemRecipeBuilder.stonecutting(Ingredient.of(blockCobble), blockCobbleStairs)
                            .unlocks("has_stone", has(blockCobble))
                            .save(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_stairs_stonecutting"));

                    SingleItemRecipeBuilder.stonecutting(Ingredient.of(blockCobble), blockCobbleWall).unlocks("has_stone", has(blockCobble))
                            .save(consumer, new ResourceLocation(DropTheRock.MODID, nameCobble + "_wall_stonecutting"));
                }
            } catch (Exception e) {
...
//            mattock(consumer, "iceandfire", DTRItemRegistry.IAF_DRAGONBONE_MATTOCK.get(), IafItemRegistry.DRAGON_BONE);
    ...
    private static void knife(Consumer<IFinishedRecipe> consumer, String modLoaded, Item knife, Item mat)
    {
        ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modLoaded))
                .addRecipe(ShapedRecipeBuilder.shaped(knife).group("knife")
                        .unlockedBy("has_material", has(mat))
                        .define('#', mat).define('/', Tags.Items.RODS_WOODEN)
                        .pattern("#")
                        .pattern("/")::save).generateAdvancement()
                .build(consumer, new ResourceLocation(knife.toString()));

    }

    private static void saw(Consumer<IFinishedRecipe> consumer, String modLoaded, Item saw, Item mat)
    {
        ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modLoaded))
                .addRecipe(ShapedRecipeBuilder.shaped(saw).group("saw")
                        .unlockedBy("has_material", has(mat))
                        .define('#', mat).define('/', Tags.Items.RODS_WOODEN)
                        .pattern("  /")
                        .pattern(" /#")
                        .pattern("/#")::save).generateAdvancement()
                .build(consumer, new ResourceLocation(saw.toString()));

    }

    private static void mattock(Consumer<IFinishedRecipe> consumer, String modLoaded, Item saw, Item mat)
    {
        ConditionalRecipe.builder().addCondition(new ModLoadedCondition(modLoaded))
                .addRecipe(ShapedRecipeBuilder.shaped(saw).group("saw")
                        .unlockedBy("has_material", has(mat))
