    ...
    private static void getTierToJson(CommandSourceStack commandSource, Entity entity) throws CommandSyntaxException {
        if (entity instanceof LivingEntity) {
            ...
            if (itemstack.getItem() instanceof TieredItem) {
                ...
                try (JsonWriter writer = new JsonWriter(new FileWriter(FMLPaths.CONFIGDIR.get().resolve("dropthemat" + File.separator + Objects.requireNonNull(tier.getRepairIngredient().getItems()[0].getItem().getDescriptionId()).split(":")[1] + ".json").toFile()))) {
                    writer.setIndent("  ");
                    writer.beginObject();
                    ...
                    writer.name("RepairIngredient").value(tier.getRepairIngredient().getItems()[0].getItem().getDescriptionId());
                    writer.endObject();
                    writer.close();
                    DropTheRock.LOGGER.info("Wrote material file in dropthemat/{}.json", tier.getRepairIngredient().getItems()[0].getItem().getDescriptionId().split(":")[1]);
                } catch (Exception e) {
